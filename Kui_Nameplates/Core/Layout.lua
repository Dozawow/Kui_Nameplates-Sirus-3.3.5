...
local function SizerOnSizeChanged(self, x, y)
	...
	self.f.MOVING = true
	self.f:Hide()
	self.f:SetPoint("CENTER", WorldFrame, "BOTTOMLEFT", floor(x), floor(y + 10))
	self.f:Show()
	self.f.MOVING = nil
end
------------------------------------------------------------- Frame functions --
...
local function UpdateFrame(self)
    ...
    if self.guid and self.spell and self.state and not self.health.curr then
        self.health:Hide()
        self.bg:Hide()
        self.overlay:Hide()
        self.name:Hide()
...
local function UpdateFrameCritical(self)
    ...
    if self.targetDelay >= 1 then
        ...
        self:SetScale(addon.uiscale * 1.25)
		...
		if self.target then
			...
			self.target = nil
                        ...
                        self:SetScale(addon.uiscale * 1.0)
                        else
                        self:SetScale(addon.uiscale)
                        end
...
local function SetName(self)
    ...
    if self.name.text then
        self.name:SetText(utf8sub(self.name.text, 1, 14))
    end
end
