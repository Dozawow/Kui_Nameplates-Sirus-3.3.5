...
function mod:CreateCastbar(frame)
	...
	if self.db.profile.display.spellname then
		frame.castbar.name = frame:CreateFontString(frame.castbar.bar, {size = "12"})
		frame.castbar.name:ClearAllPoints()
                frame.castbar.name:SetJustifyH("LEFT")
                frame.castbar.name:SetPoint("LEFT", frame.castbar.bar, "LEFT", 0, -2)
		frame.castbar.name:SetTextColor(1, 0.82, 0)
	end

	if self.db.profile.display.casttime then
		frame.castbar.curr = frame:CreateFontString(frame.castbar.bar, {size = "12"})
