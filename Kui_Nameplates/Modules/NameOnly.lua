...
if f.testIcon then
    f.testIcon:SetParent(f)
    f.testIcon:ClearAllPoints()
    f.testIcon:SetPoint("BOTTOM", f.name, "TOP", 0, 2)
end
...
if f.testIcon then
    f.testIcon:SetParent(f.overlay)
    f.testIcon:ClearAllPoints()
    f.testIcon:SetPoint("BOTTOM", f.name, "TOP", 0, 2)
end
...
local function nameonly_SetName(f)
    ...
    if f.friend and not f.player and f.name.text then
        f.name:SetWidth(0)
        f.name:SetText(utf8sub(f.name.text, 1, 10))
        f.name:SetWidth(f.name:GetStringWidth())
        return
    end
    ...
    if not name or not f.health.curr or not f.health.max then
        return
    end

    local health_length = len(name) * (f.health.curr / f.health.max)

    f.name:SetText(
        utf8sub(name, 0, health_length) ..
        "|cff666666" ..
        utf8sub(name, health_length + 1)
    )
end  -- ❗ ВОТ ЭТОГО У ТЕБЯ НЕ БЫЛО
...
function mod:PostCreate(msg, f)
    ...
    if f.SetName ~= nameonly_SetName then
        HookSetName(f)
    end
end
