...
local function SetCastWarning(self, spellName, spellSchool)
	self.castWarning:Stop()
		...
		self.castWarning:SetText(spellName)
                self.castWarning:SetTextColor(1, 0.82, 0)
...
function mod:CreateCastWarnings(msg, frame)
	-- casting spell name
	frame.castWarning = frame:CreateFontString(frame.castbar.bar, {size = "12"})
        frame.castWarning:SetTextColor(1, 0.82, 0)

        frame.castWarning:Hide()
        frame.castWarning:ClearAllPoints()
        frame.castWarning:SetJustifyH("LEFT")
        frame.castWarning:SetPoint("LEFT", frame.castbar.bar, "LEFT", 0, -2)
...
function mod:OnInitialize()
	self.db = addon.db:RegisterNamespace(self.moduleName, {profile = {warnings = true, useNames = true}})
